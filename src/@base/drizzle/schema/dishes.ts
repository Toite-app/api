...
export const dishes = pgTable(
  "dishes",
  {
    ...
    weightMeasure: weightMeasureEnum("weight_measure")
      .notNull()
      .default("grams"),
  ...
  (t) => [
    index("dishes_menu_id_idx").on(t.menuId),
    index("dishes_is_published_in_app_idx").on(t.isPublishedInApp),
    index("dishes_is_published_at_site_idx").on(t.isPublishedAtSite),
  ],
);
...
export const dishesToDishCategories = pgTable(
  "dishes_to_dish_categories",
  {
    dishId: uuid("dish_id").notNull(),
...
export type IDishToDishCategories = typeof dishesToDishCategories.$inferSelect;

export const dishesToDishCategoriesRelations = relations(
  dishesToDishCategories,
  ({ one }) => ({
    dish: one(dishes, {
      fields: [dishesToDishCategories.dishId],
      references: [dishes.id],
    }),
    dishCategory: one(dishCategories, {
      fields: [dishesToDishCategories.dishCategoryId],
